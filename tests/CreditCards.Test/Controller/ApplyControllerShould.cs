...
namespace CreditCards.Test.Controller
{
    public class ApplyControllerShould
    {
        readonly Mock<ICreditCardApplicationRepository> _mockRepository;
        readonly ApplyController _sut;
        ...
        public async Task SaveApplicationWhenValidModel()
        {
            ...
            _mockRepository.Setup(x => x.AddAsync(It.IsAny<CreditCardApplication>())).Returns(Task.CompletedTask).Callback<CreditCardApplication>(x => savedApplication = x);
